...
<head>
    ...
    <title>Customer Information - Vehicle Rental</title>
...
<body class="bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto mt-10 px-4">
        <div class="grid md:grid-cols-2 gap-8">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <h2 class="text-2xl font-bold mb-6">Selected Vehicle</h2>
                <?php if (isset($_GET['name']) && isset($_GET['price']) && isset($_GET['capacity'])): ?>
                    <div class="space-y-6">
                        <img src="../VEHICLE RENTAL -DB/<?php echo htmlspecialchars($_GET['capacity']); ?> Vehicle Capacity/<?php echo htmlspecialchars($_GET['image'] ?? ''); ?>" 
                            alt="<?php echo htmlspecialchars($_GET['name']); ?>"
                            class="w-full h-48 object-cover rounded-lg mb-4">
                        <div class="space-y-4">
                            <div>
                                <p class="text-sm text-gray-500">Vehicle</p>
                                <p class="font-semibold text-lg"><?php echo htmlspecialchars($_GET['name']); ?></p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500">Capacity</p>
                                <p class="font-semibold"><?php echo htmlspecialchars($_GET['capacity']); ?> Seater</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500">Price per Day</p>
                                <p class="font-semibold text-xl text-blue-600">₱<?php echo htmlspecialchars($_GET['price']); ?></p>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <form id="customerForm" onsubmit="showPaymentModal(event)" class="space-y-6">
                    <!-- Hidden Vehicle Information -->
                    <input type="hidden" name="vehicle_id" value="<?php echo htmlspecialchars($_GET['vehicle_id'] ?? ''); ?>">
                    <input type="hidden" name="vehicle_name" value="<?php echo htmlspecialchars($_GET['name'] ?? ''); ?>">
                    <input type="hidden" name="vehicle_price" value="<?php echo htmlspecialchars($_GET['price'] ?? ''); ?>">
                    <input type="hidden" name="vehicle_capacity" value="<?php echo htmlspecialchars($_GET['capacity'] ?? ''); ?>">
                    
                    <!-- Customer Type Selection -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Customer Type</label>
                        <select name="customer_type" id="customerType" onchange="toggleCompanyField()" 
        ...
        <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4 relative">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select id="driverType" name="driver_type" class="w-full p-2 border border-gray-300 rounded-md">
    ...
    function submitFullForm() {
        ...
        if (!paymentMethod || !driverType) {
            alert('Please select both payment method and driver type');
            return;
        }

        // Create a new form with all data
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = 'transaction_details.php';
        ...
        driverInput.name = 'driver_type';
        driverInput.value = driverType;
        form.appendChild(driverInput);

        // Add vehicle data from URL parameters
        const urlParams = new URLSearchParams(window.location.search);
        ['vehicle_id', 'name', 'price', 'capacity', 'image'].forEach(param => {
            if (urlParams.has(param)) {
                const input = document.createElement('input');
                input.type = 'hidden';
                input.name = 'vehicle_' + param;
